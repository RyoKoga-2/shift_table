...
with tab3:
    ...
    if staff_file is not None and calendar_file is not None:
        ...
        if optimize_button:
            ...
            st.session_state.shift_scheduler = shift_scheduler
        if st.session_state.optimization_done:
            shift_scheduler = st.session_state.shift_scheduler
            ...
            # CSVデータをダウンロード可能にする
            csv_data = shift_scheduler.sch_df.to_csv(index=False)  # CSVデータに変換
            
            # CSVダウンロードボタンを配置
            st.download_button(
                label="シフト表をCSVでダウンロード",
                data=csv_data,
                file_name="optimized_shift_schedule.csv",
                mime="text/csv"
            )
